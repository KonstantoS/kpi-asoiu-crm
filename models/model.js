    ...
    data: function(){
        ...
        for(var key in this){
            if(this.hasOwnProperty(key)){
                if(this[key] !== null && db.schema[this._schema].hasOwnProperty(key) && !db.schema[this._schema][key].encrypted)
                    data[key] = this[key];
            }
        }
        return data;
        ...
        this.isUnique(function(err){
            if(err === null){
                if(true === isNaN(parseInt(self.id)))
                    db.insert(self._schema,self.data(),function(err, result){
                        if(err !== null)
                            return callback(err);
                        else
                    ...
                    db.update(self._schema,self.data(),[[[self._schema,'id'],'=',self.id]],function(err,result){
                        if(err !== null)
                            return callback(err);
